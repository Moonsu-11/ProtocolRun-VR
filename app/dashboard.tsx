...
export default function Dashboard() {
  ...
  const fresh = !!s?.last_seen && Date.now() / 1000 - s.last_seen < 10;
  ...
  return <div className="workspace">
    <header className="topbar"><a href="/" className="brand"><span className="brand-icon"><Workflow size={21}/></span>ProtocolRun<span className="brand-vr">VR</span></a><div className="top-meta"><span className="mono">STUDY OPERATIONS / 0.3 RC</span><Badge variant="outline">연구자 콘솔</Badge></div></header>
    <main className="main-shell">
      ...
      <div className="console-grid"><aside className="left-column">
        ...
        <Tabs defaultValue="monitor"><TabsList variant="line" className="main-tabs"><TabsTrigger value="monitor">실시간 관찰</TabsTrigger><TabsTrigger value="protocol">프로토콜</TabsTrigger><TabsTrigger value="report">분석·내보내기</TabsTrigger></TabsList>
          ...
          <TabsContent value="protocol"><section className="panel protocol-editor"><span className="eyebrow">IMMUTABLE PROTOCOL</span><h2>연구 조건을 먼저 고정하세요</h2><p>이번 구현은 장비 확인 → 연습 → 탐색 → 잡기 → 배치 → 설문 순서입니다. 아래 설정으로 새 프로토콜을 만들 수 있습니다. 이미 시작한 세션의 설정은 바뀌지 않습니다.</p><label htmlFor="protocol-json">프로토콜 JSON</label><Textarea id="protocol-json" value={editor} onChange={e => setEditor(e.target.value)} className="json-editor" spellCheck={false}/><Button onClick={saveProtocol} disabled={!connection || busy}>새 프로토콜 저장<ArrowRight/></Button><p className="hint">Meta 손 추적: A 정상, B 복구 대상, C 비대상. B의 원래 잡기 컴포넌트 상태만 복원합니다. baseline_mask는 구형 XRI 경로에만 사용합니다. demo_faults_allowed는 시연용 장애 주입 허용 여부입니다.</p></section></TabsContent>

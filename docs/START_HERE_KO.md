# ProtocolRun-VR 0.4.0 RC2 — 실제 연결 시작
...
1. 동의 → 양손 추적 확인.
2. 연습 단계에서 **A와 B를 각각 한 번 잡고 놓는다.** 서버에 정상 작동 이력을 남긴다.
3. 목표 접근 단계로 넘어가면 손을 B에서 떨어뜨린다.
4. 연구자가 Unity `ProtocolRunSession` Inspector의 `RESEARCHER DEMO: Inject B fault`를 누른다. **고의 장애 주입임을 영상에 밝힌다.** 자동/수동 복원 버튼은 누르지 않는다.

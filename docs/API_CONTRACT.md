...
- A/B practice uses actual `sdk_selection` grab and release observations. C input is `non_grabbable_attempt`, not a fault.
...
1. Gemini ADK diagnosis reads recent evidence and registered baselines. Fault injection events are withheld from the diagnostic evidence list; a demo flag is not technical proof.
2. Recovery agent proposes one bounded action with concrete failed-event IDs. Free text is never executed.
3. Firewall requires target-step/running status, permission, prior normal B practice, 3 distinct tracked near pinch failures, active collider and disabled captured direct grab paths. Old/future-dated or superseded evidence is rejected.
